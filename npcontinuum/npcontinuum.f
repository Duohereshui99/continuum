...
ccccccc
            ...
            real*8::h                       !步长
            real*8::mass_1,mass_2,mu        !组分质量和约化质量
            real*8::E,k,k2                  !能量(质心系Ecm)和波矢(及平方)
            real*8::v0,r0,a                 !gausspot势参数
            real*8::eta                     !Sommerfeld参数
            real*8::z1,z2                   !带电量
            real*8::XLMIN                   !L的最小值，一般从0开始
...
ccccccc            
            z1=1d0                         !带电量
            z2=0d0
            mass_1=1.d0                    !组分质量和约化质量
            mass_2=1.d0
            mu=amu*(mass_2*mass_1)/(mass_1+mass_2)
            E=0.29345052885026601d0!5d0                            !单位MeV
...
ccccccc
            allocate(r(0:nint))             !r
            allocate(z(0:nint))             !auxiliary wf
            allocate(y(0:nint))             !trial wf
            allocate(V(0:nint))             !V
            allocate(f(0:nint))             !f
            allocate(psi(0:nint))           !psi,final wf
            allocate(FC(0:l))               !Coulomb函数F
            allocate(GC(0:l))               !Coulomb函数G
            allocate(FCP(0:l))              !Coulomb函数F的一阶导数
            allocate(GCP(0:l))              !Coulomb函数G的一阶导数
                                            !这里存的是不同l的,不是不同位置r的
ccccccc
            do i=0,nint                     !r写在最前面
                r(i)=r_0+i*h
